-- Correct Hivemind follow-up timing and restore caster-self Vengeance.
-- Apply after fix_production_ability_access.sql.

update public.ability_catalog
set input_schema = '[]'::jsonb,
    effect_config = '{}'::jsonb - 'target'
where ability_id = 'ability_hivemind';

update public.ability_catalog
set input_schema = '[]'::jsonb,
    effect_config = jsonb_set(effect_config, '{target}', '"caster"'::jsonb, true)
where ability_id = 'ability_vengeance';
...
begin
  ...
  if duration_seconds is not null or (definition.effect_config->>'trigger_type') is not null then
    insert into ability_effects (
      ...
      jsonb_build_object('caster_user_id', auth.uid(), 'caster_display_name', caster.display_name,
        ...
        'spell_name', definition.display_name, 'spell_description', definition.description, 'ability_id', ability_id_value)
...
create or replace function public.list_pending_ability_workflows()
returns table(cast_id bigint, ability_id text, display_name text, workflow text, created_at timestamptz, variables jsonb)
language sql security definer set search_path = public as $$
  select c.id, c.ability_id, a.display_name, a.workflow, c.created_at, c.variables
  from ability_casts c join ability_catalog a on a.ability_id = c.ability_id
  where c.status = 'pending' and a.workflow in ('ready', 'commit')
    and exists (select 1 from profiles p where p.id = auth.uid() and p.is_production = true)
...
declare
  updated_cast ability_casts%rowtype;
  definition ability_catalog%rowtype;
  template_key text;
  template_value text;
  audience_value text;
...
begin
  if not exists (select 1 from profiles where id = auth.uid() and is_production = true)
  ...
  select * into definition from ability_catalog where ability_id = updated_cast.ability_id;
  template_key := case when definition.workflow = 'ready' then 'READY_TO_ALL' else 'COMMIT_SUCCESS' end;
  template_value := definition.notification_templates->>template_key;
  if template_value is not null and template_value <> '' then
    audience_value := case when definition.workflow = 'ready' then 'all' else 'user' end;
    variables_value := updated_cast.variables || jsonb_build_object(
      'display_name', updated_cast.caster_display_name,
      'spell_name', definition.display_name,
      'spell_description', definition.description,
      'assigned_camp', updated_cast.caster_camp,
      'target_player', updated_cast.caster_display_name
    );
    insert into ability_notifications (cast_id, template_key, template, audience, recipient_user_id, recipient_camp, variables)
    values (updated_cast.id, template_key, template_value, audience_value,
      case when audience_value = 'user' then updated_cast.user_id else null end,
      case when audience_value = 'camp' then updated_cast.caster_camp else null end,
      variables_value);
  end if;
...
grant execute on function public.list_pending_ability_workflows() to authenticated;
