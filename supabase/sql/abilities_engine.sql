...
alter table public.ability_catalog
  ...
  add column if not exists usage_limit_enabled boolean not null default false,
  add column if not exists usage_limit_type text not null default 'user',
...
insert into public.ability_catalog (
  ability_id, mana_cost, display_name, description, icon, usage_limit, usage_limit_enabled, usage_limit_type, usage_limit_count, usage_scope,
  input_schema, workflow, effect_config, eligibility, notification_templates
) values
('ability_telegrab', 25, 'Telegrab', 'Tell Production to retrieve an item from another camp.', '/assets/abilities/telegrab_icon.webp', 1, true, 'user', 1, 'user', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true},{"key":"item","label":"Item","type":"item","required":true}]', 'commit', '{}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] telegrabbed a [item] from [target_player].","COMMIT_SUCCESS":"Weird... something''s gone missing from your camp."}'),
('ability_hivemind', 35, 'Hivemind', 'Plan an event that everyone has to do in the center.', '/assets/abilities/hive_mind_icon.webp', 1, true, 'camp', 1, 'camp', '[]', 'ready', '{}', '{"achievement":"veteran_camper"}', '{"CAST_TO_PRODUCTION":"[assigned_camp] is planning an event.","READY_TO_ALL":"[assigned_camp] casts Hivemind. Gather the campers in the center."}'),
('ability_compulsion', 25, 'Compulsion', 'Let everyone know that you are the highest-level Grove member.', '/assets/abilities/compulsion_icon.webp', null, false, 'user', null, 'user', '[]', 'instant', '{}', '{"type":"highest_level"}', '{"CAST_TO_ALL":"[display_name] is at the market. He''s literally the best grove member out there. Maybe if you give him something good he''ll think you''re cool!"}'),
('ability_spree', 125, 'Spree', 'Everything they purchase is 1 can for the next minute.', '/assets/abilities/spree_icon.webp', null, false, 'user', null, 'user', '[]', 'instant', '{"duration_seconds":60}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description]","EFFECT_EXPIRED":"[display_name]''s Spree is over."}'),
('ability_conjure', 50, 'Conjure', 'Summon an NPC to follow and assist the caster for 15 minutes.', '/assets/abilities/conjure_icon.webp', null, false, 'user', null, 'user', '[{"key":"npc","label":"NPC","type":"npc","required":true}]', 'instant', '{"duration_seconds":900}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description]","EFFECT_EXPIRED":"[display_name]''s summoning spell has ended. Return to your normal duties."}'),
('ability_low_alch', 5, 'Low Alch', 'Return a card for 1 can.', '/assets/abilities/low_alch_icon.png', null, false, 'user', null, 'user', '[]', 'instant', '{}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description]"}'),
('ability_high_alch', 35, 'High Alch', 'Return a card for its original value.', '/assets/abilities/high_alch_icon.png', 3, true, 'user', 3, 'user', '[{"key":"item","label":"Card","type":"item","required":true}]', 'instant', '{}', '{"achievement":"shopaholic"}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description]"}'),
('ability_superheat', 10, 'Superheat', 'Exchange one item for another result.', '/assets/abilities/superheat_icon.png', null, false, 'user', null, 'user', '[{"key":"item_1","label":"Item to exchange","type":"item","required":true},{"key":"item_2","label":"Item to receive","type":"item","required":true}]', 'instant', '{}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. They are exchanging [item_1] for [item_2]."}'),
('ability_teleother', 25, 'Teleother', 'Move another camper to a chosen location.', '/assets/abilities/teleother_icon.png', null, false, 'user', null, 'camp', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true},{"key":"location","label":"Desired location","type":"location","required":true}]', 'instant', '{}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. Move [target_player] to [location]."}'),
('ability_vengeance', 25, 'Vengeance', 'The next spell that targets the caster is reflected back.', '/assets/abilities/vengeance_icon.png', null, false, 'user', null, 'user', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true}]', 'instant', '{"duration_seconds":3600,"target":"target","trigger_type":"targeted_ability"}', '{"achievement":"high_score"}', '{"CAST_TO_ALL":"[display_name] has armed [spell_name]. [spell_description]","REFLECTED":"Your spell was reflected by [target_player]."}'),
('ability_ice_barrage', 25, 'Ice Barrage', 'Freeze a player for five minutes.', '/assets/abilities/ice_barrage_icon.png', null, false, 'user', null, 'user', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true}]', 'instant', '{"duration_seconds":300}', '{}', '{"CAST_TO_TARGET":"[display_name] has frozen you with [spell_name]. Do NOT move for 5 minutes.","CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description].","EFFECT_EXPIRED":"[target_player] has thawed."}'),
('ability_drop_party', 50, 'Drop Party', 'Gather the NPCs and value items for a drop party in the center.', '/assets/abilities/drop_party_icon.png', null, false, 'user', null, 'camp', '[]', 'ready', '{}', '{}', '{"CAST_TO_PRODUCTION":"[display_name] has cast [spell_name]. [spell_description].","READY_TO_ALL":"The drop party is happening in the center."}'),
('ability_love_blast', 25, 'Love Blast', 'Make your target serenade the caster.', '/assets/abilities/love_potion_icon.webp', null, false, 'user', null, 'user', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true}]', 'instant', '{}', '{}', '{"CAST_TO_TARGET":"[display_name] hit YOU with [spell_name]. [spell_description]"}'),
('ability_necromancy', 25, 'Necromancy', 'Make your target walk like a zombie through each camp.', '/assets/abilities/necromancy_icon.webp', null, false, 'user', null, 'user', '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true}]', 'instant', '{}', '{}', '{"CAST_TO_TARGET":"[display_name] hit YOU with [spell_name]. [spell_description]"}')
on conflict (ability_id) do update set
  ...
  icon = excluded.icon, usage_limit = excluded.usage_limit, usage_limit_enabled = excluded.usage_limit_enabled,
  usage_limit_type = excluded.usage_limit_type, usage_limit_count = excluded.usage_limit_count, usage_scope = excluded.usage_scope,
...
begin
  ...
  if duration_seconds is not null or (definition.effect_config->>'trigger_type') is not null then
    insert into ability_effects (
      ...
      jsonb_build_object(
        'caster_user_id', auth.uid(),
        'caster_display_name', caster.display_name,
        'target_user_id', target_user_id_value,
        'target_display_name', target_display_name_value,
        'display_name', caster.display_name,
        'target_player', target_display_name_value,
  ...
  select cast_id, effect_id, key, value, case
    ...
    jsonb_build_object(
      ...
      'caster_display_name', caster.display_name,
      'target_display_name', target_display_name_value,
      'item', variables_value->>'item',
      'location', variables_value->>'location',
      'npc', variables_value->>'npc',
      'assigned_camp', variables_value->>'assigned_camp',
...
begin
  ...
  with expired as (
    update ability_effects
      ...
      returning id, cast_id, ability_id, caster_user_id, caster_display_name, target_user_id, target_display_name, metadata
  )
  insert into ability_notifications (cast_id, effect_id, template_key, template, audience, recipient_user_id, variables)
    select expired.cast_id, expired.id, 'EFFECT_EXPIRED',
      ...
      jsonb_build_object(
        'display_name', coalesce(expired.caster_display_name, caster.display_name, 'Grove member'),
        'target_player', coalesce(expired.target_display_name, target.display_name, 'the target'),
        'target_display_name', coalesce(expired.target_display_name, target.display_name),
        'caster_display_name', coalesce(expired.caster_display_name, caster.display_name),
        'caster_user_id', expired.caster_user_id,
        'target_user_id', expired.target_user_id,
        'spell_name', c.display_name,
        'spell_description', c.description,
        'ability_id', expired.ability_id
      )
    from expired
    join ability_catalog c on c.ability_id = expired.ability_id
    left join profiles caster on caster.id = expired.caster_user_id
    left join profiles target on target.id = expired.target_user_id;
