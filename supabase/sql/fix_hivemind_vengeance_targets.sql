-- Additive correction for Hivemind and Vengeance target behavior.
-- Apply after fix_ability_mana_deduction.sql and fix_profile_authority_mana_cast.sql.
-- Existing mana, limits, effects, notifications, and production workflows are retained.
...
set input_schema = '[{"key":"target_player","label":"Target player","type":"player","required":true,"exclude_self":true}]'::jsonb,
    effect_config = jsonb_set(effect_config, '{target}', '"target"'::jsonb, true)
  ...
  loop
    ...
    if required_input->>'type' = 'player' then
      if target_user_id_value is null then
        raise exception 'A target player is required';
      end if;
