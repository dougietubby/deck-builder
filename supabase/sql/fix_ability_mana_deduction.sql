-- Additive mana fix for the existing ability engine.
-- The progression protection trigger was restoring mana during cast_ability().
-- The cast RPC marks its transaction with a private local setting so the trigger
-- can distinguish that server-authoritative update from a client profile update.
